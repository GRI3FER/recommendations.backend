...
function getAdminPanelUrl() {
    const fallback = 'https://your-site.com/endorsements-admin.html';
    const configuredUrl = (process.env.ADMIN_URL || '').trim();
    ...
    try {
        ...
        if (parsedUrl.pathname.toLowerCase().endsWith('/endorsements-admin.html')) {
            return parsedUrl.toString();
        }

        parsedUrl.pathname = parsedUrl.pathname.replace(/\/$/, '') + '/endorsements-admin.html';
        return parsedUrl.toString();
    } catch {
        return configuredUrl.replace(/\/$/, '') + '/endorsements-admin.html';
    }
}
    ...
    try {
        ...
        const mailOptions = {
            ...
            html: `
                ...
                <br>
                <p><a href="${getAdminPanelUrl()}" style="background: #667eea; color: white; padding: 10px 20px; text-decoration: none; border-radius: 5px;">Review in Admin Panel</a></p>
            `
        };
